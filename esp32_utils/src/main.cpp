...
Servo servos[4];
int servoPins[] = {GPIO_SERVO1, GPIO_SERVO2, GPIO_SERVO3, GPIO_SERVO4, GPIO_SERVO5, GPIO_SERVO6};
   ...
   if (message.length() > 0) {
       ...
       if (firstComma != -1 && secondComma != -1) {
           ...
           int checksum = checksumStr.toInt();
            ...
            //Check the servo number and set the angle for the selected servo
           if (servoNumber >= 0 && servoNumber < 8) {
