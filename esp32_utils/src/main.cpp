...
void parseMessage(const String& message) {
    ...
    float speed = value.substring(0, value.indexOf(',')).toFloat(); 
...
void loop() {
    String message = readMessage();  // Leemos el mensaje
    ...
    if (message.length() > 0) {
        ...
        motorL.setpoint = motor_left_speed;  
        motorR.setpoint = motor_right_speed;  
        motor3.setpoint = motor_3_speed;  
        motor4.setpoint = motor_4_speed;  
